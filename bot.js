...
const { h2k, generateMessageID, getGroupAdmins, banner, start, info, success, close } = require('./lib/functions.js')
...
switch (command) {
        ...
        case 'google':
                ...
                let search = await googleit({ query: buscar })
                let ggsm = ``
                for (let i of search) {
                ggsm += `
                *Titulo :* ${i.title}
        ...
        El archivo se está enviando.
                ...
                sendFileFromUrl(pr21.result.url_audio, audio, {quoted: koner, mimetype: 'audio/mp4', ptt: true, filename: res1[0].output})
                                })
                sendFileFromUrl(res1[0].link, audio, {quoted: koner, mimetype: 'audio/mp4', filename: res1[0].output})
                sendFileFromUrl(res1[0].link, audio, {quoted: koner, mimetype: 'audio/mp4', ptt: true, filename: res1[0].output})
                                }
                addFilter(from)
                        break
                }
